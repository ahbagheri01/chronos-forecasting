...
2. The WQL plots do not use the official WQL aggregation. `plot_results.py` plots the simple
   mean of each series' WQL, while `compare_summary.csv` correctly computes total quantile
   loss divided by total absolute target. This materially changes M1 yearly: the official top
   three are DeepAR, Prophet, and ARIMA, but the WQL plot shows ARIMA, ARMA, and AutoARIMA.
   Do not use any WQL PNG for exact values or ranking; use `compare_summary.csv`.
...
The WQL PNGs do not agree with official aggregation and should be considered diagnostic only.
...
The normalized and dot-comparison plots apply per-metric min-max scaling. A value of zero means
"best model in this set," not zero error, and distances change if any model is added or removed.
Outliers compress useful differences: MA(2) flattens M1 WQL, MA(2)/Prophet compress PSM, and
weather's SMAPE/WQL pathologies distort that panel. External PSM bar labels rounded to two
decimals can display distinct small values as `0.00`. Use the CSV tables above for decisions.

## Recommended Next Actions

1. Fix PSM timestamps and define whether the desired horizon is 24 minutes, 24 hours, or
   another interval; then rerun with a matching seasonal lag.
2. Fix the plot generator to use the same WQL aggregation as `metrics.summarize` and regenerate
   all WQL/normalized plots.
3. Add an exact-zero baseline and occurrence/positive-amount metrics for weather; persist point
   forecasts so physical validity can be audited without rerunning models.
4. Use rolling-origin evaluation and multiple stochastic seeds, especially for DeepAR, Chronos,
   and Prophet. Report dispersion or confidence intervals, not only one cutoff.
5. Evaluate random or stratified subsets of all traffic sensors and PSM features instead of only
   the first five columns.
6. Keep seasonal naive as a mandatory baseline, use ARIMA(2,1,2) as the strongest classical
   baseline, and remove MA(2) from serious model selection unless its purpose is purely didactic.
