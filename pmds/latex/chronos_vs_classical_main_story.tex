...
% -----------------------------------------------------------------------------
\begin{frame}{Hypothesis tests and confidence sets for predictive ability}
  \vspace{0.08cm}
  \begin{center}
    \begin{tikzpicture}[
      node distance=5mm,
      box/.style={draw=Line,rounded corners=2pt,align=left,text width=3.80cm,
        minimum height=4.65cm,inner sep=8pt},
      arrow/.style={-{Stealth[length=5pt]},thick,color=Muted}
    ]
      \node[box,fill=TealLight] (dm) {
        \large\textbf{Diebold--Mariano}\\[4pt]
        \scriptsize
        $d_t=L_t(A)-L_t(B)$\\[-1pt]
        $H_0:\;\mathbb{E}[d_t]=0$\\[5pt]
        Tests equal predictive accuracy for two models while estimating the
        sampling variance of the serially dependent loss differential.\\[5pt]
        \textbf{Insight:} direction and significance of one pairwise difference.
      };
      \node[box,fill=GoldLight,right=of dm] (spa) {
        \large\textbf{SPA}\\[4pt]
        \scriptsize
        $d_{k,t}=L_t(B)-L_t(k)$\\[-1pt]
        $H_0:\;\max_k\mathbb{E}[d_{k,t}]\leq 0$\\[5pt]
        Tests whether any candidate improves on benchmark $B$. Bootstrap
        critical values account for searching over many alternatives.\\[5pt]
        \textbf{Insight:} evidence that the benchmark is genuinely outperformed.
      };
      \node[box,fill=PurpleLight,right=of spa] (mcs) {
        \large\textbf{Model Confidence Set}\\[4pt]
        \scriptsize
        $H_0:\;\mathbb{E}[d_{ij,t}]=0$ for all $i,j\in\mathcal{M}$\\[5pt]
        Begins with all models and sequentially removes the worst-supported
        member until equal predictive ability is no longer rejected.\\[5pt]
        \textbf{Insight:} $\widehat{\mathcal{M}}_{1-\alpha}$, the set of models
        statistically compatible with best performance.
      };
      \draw[arrow] (dm) -- (spa);
      \draw[arrow] (spa) -- (mcs);
    \end{tikzpicture}
