...
% ------------------------------------------------------------
\begin{frame}{Six datasets, nine models, one common pipeline}
  \vspace{0.20cm}
  ...
  \begin{columns}[T,totalwidth=0.96\linewidth]
    ...
    \begin{column}{0.42\linewidth}
      ...
      \vspace{0.22cm}
      {\color{Muted}\scriptsize CPU execution; point metrics use the median forecast.}
    \end{column}
  \end{columns}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}{There is no single winner across all forecasting tasks}
  \vspace{0.08cm}
  \scriptsize
  ...
  \begin{center}
    \begin{tabular}{L{0.23\linewidth} L{0.26\linewidth} L{0.18\linewidth} L{0.25\linewidth}}
      ...
      Weather
        & \bad{No clean winner}
        & Chronos, narrowly
        & Chronos point forecast is zero-like \\
      National illness
        & \good{ARIMA(2,1,2)}
        & \good{Prophet}
        & One series, one cutoff \\
      PSM
        & \good{ARIMA(2,1,2)}
        & \good{Chronos}
        & Minute data labeled hourly \\
      Traffic
        & \good{DeepAR}
        & \good{DeepAR}
...
\begin{frame}{Weather: the apparent winner predicts almost exactly zero}
  ...
  \begin{columns}[T,totalwidth=\linewidth]
    \begin{column}{0.58\linewidth}
      \includegraphics[width=\linewidth,height=0.48\textheight,keepaspectratio]{chronos_weather/smape.png}
    \end{column}
    \begin{column}{0.38\linewidth}
      \scriptsize
      \tagbox{CoralLight}{\color{Coral}\textbf{Metric pathology}}
      ...
      \vspace{0.16cm}
      \tagbox{GoldLight}{\textbf{Why MAE still looks good}}

      \vspace{0.12cm}
      Rainfall is 72.67\% zero. An exact-zero forecast matches
      Chronos MAE, RMSE, and MASE to floating-point precision.

      \vspace{0.14cm}
      \begin{itemize}
        \item Chronos WQL 0.984 vs. zero baseline 1.000.
        \item DeepAR has the best RMSE, 1.965.
        \item Add rain-occurrence and positive-amount metrics.
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T,totalwidth=\linewidth]
    ...
    \begin{column}{0.36\linewidth}
      ...
      \begin{itemize}
        ...
        \item Only one \texttt{OT} series and one cutoff were tested.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}{PSM: strong numbers, wrong operational time scale}
  \vspace{0.03cm}
  \begin{columns}[T,totalwidth=\linewidth]
    ...
    \begin{column}{0.37\linewidth}
      \scriptsize
      \tagbox{GreenLight}{\textbf{Numerical ranking}}
      ...
      \vspace{0.16cm}
      \tagbox{CoralLight}{\color{Coral}\textbf{Configuration mismatch}}

      \vspace{0.12cm}
      Source \texttt{timestamp\_(min)} increments by one minute,
      but the project code creates hourly timestamps and uses lag 24.

      \vspace{0.14cm}
      \begin{itemize}
        \item This is a 24-step result, apparently 24 minutes.
        \item Large MASE values reflect tiny lag-24 scales.
        \item Five of 25 features were evaluated.
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \vspace{0.02cm}
  \centering
  \tagbox{RedLight}{\color{Red}\scriptsize\textbf{Do not select a PSM model until the time scale is corrected and rerun.}}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}{Descriptive scorecard: ARIMA ranks first, Chronos second}
  \vspace{0.10cm}
  \scriptsize
...
% ------------------------------------------------------------
\begin{frame}{What the comparison supports, and what it does not}
  \vspace{0.12cm}

  \begin{columns}[T,totalwidth=0.94\linewidth]
    \begin{column}{0.44\linewidth}
      \small
      \tagbox{GreenLight}{\textbf{What the comparison shows}}
    ...
    \begin{column}{0.44\linewidth}
      \small
      \tagbox{CoralLight}{\color{Coral}\textbf{Project constraints}}

      \vspace{0.20cm}
      \begin{itemize}
        \item One final holdout window per series.
        \item Only one illness target and five external columns.
        \item No repeated stochastic seeds or confidence intervals.
        \item M1 scale concentration changes the winner.
        \item Weather and PSM require evaluation fixes.
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \vspace{0.28cm}
  \centering
  \tagbox{GoldLight}{\textbf{Use these rankings for the configured datasets and cutoffs only.}}
\end{frame}
...
\begin{frame}{How to choose a model, and what to fix next}
  ...
  \begin{columns}[T,totalwidth=0.96\linewidth]
    ...
    \begin{column}{0.22\linewidth}
      ...
      \vspace{0.12cm}
      \scriptsize Repair evaluation first
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T,totalwidth=0.94\linewidth]
    ...
    \begin{column}{0.44\linewidth}
      \small
      \textbf{Expand the comparison}
      \begin{itemize}
        \item Use rolling origins and multiple seeds.
        \item Sample sensors/features rather than first columns.
        \item Persist forecasts and test interval calibration.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
