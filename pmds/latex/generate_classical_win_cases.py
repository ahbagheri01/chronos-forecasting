"""Generate examples where a classical model beats every foundation model."""
...
CASES = (
    {
        ...
        "dataset_title": "M1 Yearly",
        "metric": "wql",
        "metric_label": "WQL",
        "winner": "arima_2_1_2",
        "comparator": "chronos_t5_tiny",
        "item_id": "84:target::origin=1",
        "output": "m1-wql-classical-win.png",
        "history": 18,
        "gap_text": "12.4% lower",
        "intervals": True,
    },
    {
        "dataset": "official_bls_macro",
        "dataset_title": "BLS Macro",
        "metric": "mase",
        "metric_label": "MASE",
        "winner": "auto_arima",
        "comparator": "timesfm_2_5_200m",
        "item_id": "LNS14000000::origin=0",
        "output": "bls-mase-classical-win.png",
        "history": 24,
        "gap_text": "18.8% lower",
        "intervals": False,
    },
...
) -> tuple[list[tuple[int, float]], dict[int, float], dict[str, dict[str, dict[int, float]]]]:
    dataset = str(case["dataset"])
    item_id = str(case["item_id"])
    selected = {str(case["winner"]), str(case["comparator"])}
    context = [
...
def plot_case(
    ...
    dataset, metric = str(case["dataset"]), str(case["metric"])
    winner, comparator = str(case["winner"]), str(case["comparator"])
    rows = sorted(
        (row for row in repetition_rows if row["dataset"] == dataset and row["metric"] == metric),
    ...
    metric_ax.text(
        0.98, 0.94, str(case["gap_text"]), transform=metric_ax.transAxes,
        ha="right", va="top", fontsize=9, color=gold, weight="bold",
    )
